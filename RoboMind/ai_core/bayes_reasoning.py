...
def update_belief_map(belief_map: Dict[Tuple[int, int], float],
    ...
    for position, prior in belief_map.items():
        if sensor_reading:  # sensor says obstacle
            likelihood_h = sensor_accuracy              # P(reading|obstacle)
            likelihood_not_h = 1.0 - sensor_accuracy    # P(reading|free)
        else:  # sensor says free
            likelihood_h = 1.0 - sensor_accuracy        # P(no reading|obstacle)
            likelihood_not_h = sensor_accuracy          # P(no reading|free)
